...
async function openActualGraph(page) {
...
async function assertTwoWeeklyTables(page, monday, view = 'weekly') {
  const tables = page.locator('#' + view + 'Summary table');
  const periods = page.locator('#' + view + 'Summary .summary-period:not(.summary-units)');
  ...
  for (let week = 0; week < 2; week++) {
    ...
    for (const [index, date] of expected.entries()) {
      ...
      else assert.ok(cells[0].trim(), 'visible date:' + date);
    }
    dates.push(...actual);
  }
...
async function run() {
  ...
  try {
    ...
    assert.equal(await menu.locator('[data-app="black_garlic"]').isVisible(), false);
    ...
    assert.equal(await menu.locator('[data-app="black_garlic"]').count(), 1);
    assert.ok((await menu.locator('[data-app="black_garlic"]').getAttribute('href')).startsWith(appUrl));
    assert.equal(await menu.locator('[data-app="rice_shipping"]').evaluate(card => card.nextElementSibling), null);
    for (const width of [320, 390, 943, 1280]) {
      ...
      if (artifacts) await menu.screenshot({ path: path.join(artifacts, 'menu-black-garlic-github-' + width + '.png'), fullPage: true });
    }
    ...
    for (const type of ['All', 'type']) {
      ...
      for (const view of ['weekly', 'monthly']) {
        ...
        if (view === 'weekly') {
          ...
          assert.equal(await app.locator('#monthlySummary table').count(), 1);
          await app.locator('[data-summary-view="monthlyStorage"]').click();
          assert.equal(await app.locator('#monthlyStorageSummary table').count(), 1);
          const storage = app.locator('#monthlyStorageSummary [data-summary-date="2026-09-11"]');
    ...
    await openActualGraph(rooms.page);
    ...
    assert.deepEqual(await graph.page.locator('#summaryGraph .graph-series-controls select').evaluateAll(selects => selects.map(select => select.value)), ['bar', 'bar', 'line']);
    ...
    assert.deepEqual(await graph.page.locator('#summaryGraph .graph-series-controls select').evaluateAll(selects => selects.map(select => select.value)), ['bar', 'bar', 'bar']);
    ...
    assert.equal(await graph.page.locator('#forecastGraphs').isVisible(), true);
    assert.equal(await graph.page.locator('#summaryGraph').isVisible(), false);
    ...
    await graph.page.locator('#graphFullscreenBtn').click();
    await graph.page.locator('[data-summary-view="monthly"]').evaluate(button => button.click());
    assert.equal(await graph.page.locator('#graphFullscreenDialog').evaluate(dialog => dialog.open), false);
    assert.equal(await graph.page.locator('#summaryPanel #summaryGraph').count(), 1);
    assert.equal(await graph.page.locator('#monthlySummary').isVisible(), true);
    assert.equal(await graph.page.locator('#summaryMetricControls').isVisible(), true);
    await openActualGraph(graph.page);
    await graph.page.locator('#graphFullscreenBtn').click();
    ...
    for (const metric of ['out', 'in', 'empty', 'inventory']) {
      ...
      await future.page.locator('[data-summary-view="monthlyStorage"]').click();
      assert.ok((await future.page.locator('#monthlyStorageSummary [data-summary-date="2026-09-13"] td').allTextContents()).every(cell => cell.trim() === ''));
    ...
    await future.page.locator('[data-summary-view="monthlyStorage"]').click();
    assert.deepEqual(await future.page.locator('#monthlyStorageSummary [data-summary-date="2026-09-13"] .cell-upper').allTextContents(), ['9', '9']);
    assert.deepEqual(await future.page.locator('#monthlyStorageSummary [data-summary-date="2026-09-13"] .cell-lower').allTextContents(), ['5', '5']);
    await future.page.locator('[data-summary-view="weekly"]').click();
    ...
    results.singleRowRoomAndTypeMastersReadableMobileDesktopDatesWeekdaysPickerTargetsFallbackAndNoWrites = await require('./single-row-masters.cjs')(browser, scenario, unlocked);
    ...
    results.roomGraphCapacityRemovedStockAxisFiltersAndMasterSettingsPreserved = await require('./room-graph.cjs')(browser, scenario, unlocked);
    ...
    for (const role of ['operator', 'viewer']) {
      ...
      assert.equal(await test.page.locator('[data-app="black_garlic"]').count(), 1);
      assert.equal(await test.page.locator('[data-app="black_garlic"]').isVisible(), true);
      assert.ok((await test.page.locator('[data-app="black_garlic"]').getAttribute('href')).startsWith(appUrl));
      assert.equal(await test.page.locator('#noApps').isVisible(), false);
      assert.deepEqual(test.backend.errors, []);
      results['githubMenuFor' + role] = true;
    ...
    for (const [name, options] of [
      ...
      if (name === 'noAppPermission') {
        ...
        assert.equal(await test.page.locator('[data-app="black_garlic"]').isVisible(), false);
      }
      results[name] = true;
      await test.context.close();
    }
